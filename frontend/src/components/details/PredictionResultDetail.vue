<template>
  <div class="result-detail">
    ...
    <!-- BLAST NEAREST EXPERIMENTAL-Km HITS -->
    <section class="rd-section rd-collapsible-section">
      <button
        type="button"
        class="rd-collapse-header"
        :class="{ open: showBlastKmHits }"
        @click="showBlastKmHits = !showBlastKmHits"
      >
        <span class="rd-collapse-left">
          ...
          <span class="rd-collapse-text">
            <span class="rd-collapse-title">
              BLAST nearest hits with direct experimental K<sub>m</sub>
            </span>
            <span class="rd-collapse-subtitle">
              Entries with similar sequence from CarboDB with measured K<sub>m</sub> values
            </span>
          </span>
        </span>

        <span class="rd-collapse-action">
          {{ showBlastKmHits ? 'Collapse' : 'Click to expand' }}
        </span>
      </button>

      <div v-if="!showBlastKmHits" class="rd-collapse-preview">
        <template v-if="blastKmHits.length > 0">
          {{ blastKmHits.length }} BLAST hit{{ blastKmHits.length === 1 ? '' : 's' }}
          with direct experimental K<sub>m</sub> available.
        </template>

        <template v-else>
          No BLAST hit with direct experimental K<sub>m</sub> was found.
        </template>
      </div>

      <div v-if="showBlastKmHits" class="rd-collapsible-body">
        <div v-if="!blastKmHits || blastKmHits.length === 0" class="rd-neighbor-empty">
          No BLAST hit with direct experimental K<sub>m</sub> was found for EC
          <strong>{{ result.ec_predicted || result.ec_known || result.ec_number || '—' }}</strong>.
          Either this EC class has no experimental-K<sub>m</sub> BLAST database, or the submitted
          sequence did not match any reference above the significance threshold.
        </div>

        <div v-else class="rd-neighbor-list">
          <div
            v-for="sim in blastKmHits"
            :key="sim.uniprot_id"
            class="rd-neighbor-card"
            :class="'rd-tier-' + (sim.tier || 'blast_experimental_km')"
          >
            <div class="rd-neighbor-head">
              ...
              <span class="rd-neighbor-tier">
                {{ sim.tier_label || 'BLAST hit · experimental Km' }}
              </span>
            ...
            <div class="rd-neighbor-identity">
              ...
              <span class="rd-identity-label">
                ...
                <span class="rd-identity-meta">
                  ...
                  <template v-if="sim.bitscore != null">
                    <span v-if="sim.evalue != null || sim.align_length != null"> · </span>
                    bitscore {{ sim.bitscore.toFixed(1) }}
                  </template>
                </span>
              </span>
            </div>
...
const showBlastKmHits = ref(true)

const blastKmHits = computed(() => {
  return props.result?.top_similar || []
})

const topKmHit = computed(() => {
  return blastKmHits.value.find(
